...
export default {
   ...
   mounted() {
      ...
      if (this.page?.hideCenterNavigation) {
         this.$root.$emit('hideCenterNav', this.page.hideCenterNavigation)
      }
   },
   computed: {
      page() {
   ...
   async asyncData({ $graphql, payload, params }) {
      if (payload) {
         ...
         const page = await $graphql.default.request(generalPageQl, {
            slug: params.parent + '/' + params.slug,
         })
         return {
            page: page.generalPagesCollection.items[0],
         }
      }
   },
}
